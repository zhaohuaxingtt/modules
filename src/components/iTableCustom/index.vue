  ...
  methods: {
      ...
      if (this.treeExpand) {
        ...
        this.tableData.forEach((e, index) => {
          ...
          if (
            ...
            this.defaultSelectedRows.filter(d => d[this.rowKey] === e[this.rowKey]).length > 0
          ) {
            e.checked = true
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (this.customSelection) {
          ...
          if (this.defaultSelectedRows) {
            if (this.defaultSelectedRows.filter(e => e[this.rowKey] === row[this.rowKey]).length > 0) {
    ...
    toggleRowSelection(row, selected) {
      ...
      if (this.rowKey) {
        ...
        if (selected && this.selectedRows.filter(e => e[this.rowKey] === toggleRow[this.rowKey]).length === 0) {
    ...
    handleSelectable(row) {
      if (this.disableChildrenSelection && row.uniqueId && row.uniqueId.indexOf('-') > 0) {
        return false
      }
