...
export default {
  ...
  methods: {
    getSupplierInfo() {
      ...
      .then(res => {
        if (res.code == 200) {
          ...
          this.tableListData = this.tableListData.filter(item=>item.isActive)
        } else {
    ...
    handleAdd() {
      ...
      .then(res => {
        if (res.code == 200) {
          window.open(`${ window.origin }/portal/#/supplier/supplierDetail?supplierToken=${ res.data }&supplierType=4`,'_blank')
        }
      })
      .catch(() => {})
