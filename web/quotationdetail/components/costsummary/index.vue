...
<template>
  ...
  <div v-else>
    ...
    <div
      v-if="
        partInfo.partProjectType === partProjTypes.DBLINGJIAN ||
        partInfo.partProjectType === partProjTypes.DBYICHIXINGCAIGOU ||
        partInfo.priceStatus == 'DB'
      "
    >
      <quotationAnalysis
        :disabled="disabled || isOriginprice"
        :partInfo="partInfo"
    ...
    <div class="cost" v-else>
      ...
      <persentComponents
        ...
        :cbdlist="cbdlist"
        :isSteel="isSteel"
        :roundIsOnlineBidding="roundIsOnlineBidding"
        :quotationId="partInfo.quotationId"
        :tableData="topTableData"
        :disabled="disabled || isOriginprice"
        :allTableData="allTableData"
        :partType="partInfo.partType"
        :partProjectType="partInfo.partProjectType"
        :showTitle="isSkdLc"
        :isAutoCal.sync="isAutoCal"
      ></persentComponents>
      <!--------------------------------------------------------->
      <!----------------------2.1 原材料/散件--------------------->
      <!--------------------------------------------------------->
      <tableTemlate
        ...
        class="margin-top20"
        :title="`${allTableData.level}.1 ${language(
          'LK_YUANCLSJ',
          '原材料/散件'
        )}`"
        :tableTile="titleYcl"
      ...
      >
        <template #header-control>
          <div v-if="!disabled && !isOriginprice">
            <iButton @click="handleAddByRawMaterial">{{
              language("LK_TIANJIAHANG", "添加行")
            }}</iButton>
            <iButton @click="handleDelByRawMaterial">{{
      ...
      <tableTemlate
        ...
        class="margin-top20"
        :title="`${allTableData.level}.2 ${language(
          'LK_ZHIZHAOCB',
          '制造成本'
        )}`"
        :tableTile="titleCbzz"
      ...
      >
        <template #header-control>
          <div v-if="!disabled && !isOriginprice">
            <iButton @click="handleAddByMakeCost">{{
              language("LK_TIANJIAHANG", "添加行")
            }}</iButton>
            <iButton @click="handleDelByMakeCost">{{
              language("LK_SHANCHUHANG", "删除行")
            }}</iButton>
          </div>
        </template>
      ...
      <el-row class="row" v-show="allTableData.level == 2">
        <el-col class="col" :span="12">
          <tableTemlate
            :notEdit="disabled || isOriginprice"
            :tableData="allTableData.discardCost"
            class="margin-top20"
            :index="true"
            :title="`${allTableData.level}.3 ${language(
              'LK_BAOFEICHENGBEN',
              '报废成本'
            )}`"
            :tableTile="titlebfcb"
            tableIndexString="S"
            @handleInput="handleInputByDiscardCostL2"
          ></tableTemlate>
        </el-col>
        <el-col class="col" :span="12">
          <tableTemlate
            :notEdit="disabled || isOriginprice"
            :tableData="allTableData.manageFee"
            class="margin-top20"
            :index="true"
            :title="`${allTableData.level}.4 ${language(
              'LK_GUANLIFEI',
              '管理费'
            )}`"
            :tableTile="titleglf"
            tableIndexString="O"
            @handleInput="handleInputByManageFeeL2"
          ></tableTemlate>
        </el-col>
      </el-row>
      <el-row class="row" v-show="allTableData.level == 2">
        <el-col class="col" :span="12">
          <tableTemlate
            :notEdit="disabled || isOriginprice"
            :tableData="allTableData.otherFee"
            class="margin-top20"
            :index="true"
            :title="`${allTableData.level}.5 ${language(
              'LK_QITAFEIYONG',
              '其他费用'
            )}`"
            :tableTile="titleqtfy"
        ...
        <el-col class="col" :span="12">
          <tableTemlate
            :notEdit="disabled || isOriginprice"
            :tableData="allTableData.profit"
            class="margin-top20"
            :index="true"
            :title="`${allTableData.level}.6 ${language('LK_LIRUN', '利润')}`"
            :tableTile="titlelr"
            tableIndexString="P"
            @handleInput="handleInputByProfitL2"
          ></tableTemlate>
        </el-col>
      </el-row>
      <!--------------------------------------------------------->
      ...
      <el-row class="row" v-show="allTableData.level == 3">
        ...
        <el-col class="col" :span="12">
          <tableTemlate
            ...
            :tableTile="titleglfByL3"
            tableIndexString="O"
            @handleInput="handleInputByManageFeeL3"
          ></tableTemlate>
        </el-col>
      </el-row>
      <el-row class="row" v-show="allTableData.level == 3">
        <el-col class="col" :span="12">
          <tableTemlate
            ...
            :tableTile="titleqtfyByL3"
            :filterProps="{ itemType: itemTypeFilter }"
            tableIndexString="A"
          ></tableTemlate>
        </el-col>
        <el-col class="col" :span="12">
          <tableTemlate
            ...
            :index="true"
            :title="`${allTableData.level}.6 ${language('LK_LIRUN', '利润')}`"
            :tableTile="titlelrByL3"
            tableIndexString="P"
            @handleInput="handleInputByProfitL3"
          ></tableTemlate>
        </el-col>
      </el-row>
      <!--------------------------------------------------------->
      ...
      >
        <template #header-control>
          ...
          <iButton v-if="!disabled && !isOriginprice" @click="disabel">{{
            language("DELETE", "删除")
          }}</iButton>
        </template>
      </tableTemlate>
...
import {
  getCostSummary,
  packageTransport,
} from "@/api/rfqManageMent/rfqDetail";
import {
  postCostSummary,
  savePackageTransport,
  getCostSummaryDB,
  updateCostSummaryDB,
  getCategoryDetail,
...
export default {
      ...
      handler: function (val) {
        ...
        try {
          ...
          this.topTableData.persent =
            getPersent(total, this.Aprice, val[0]) || [];
        } catch (error) {
          this.topTableData.persent = [];
        }
  ...
  methods: {
        ...
        .then((res) => {
          if (res.code == 200 && res.data) {
            ...
            this.titleYcl = titleYcl(
              this.translateDicKeyCodeToName(originCountryOptions),
              materialOptions
            );
    ...
    downLoadFile() {
      ...
      // })
      if (this.cbdSelect.list.length == 0)
        return iMessage.warn("请选择cbd文件");
    ...
    disabel() {
      if (this.cbdSelect.list.length == 0)
        return iMessage.warn("请选择cbd文件");
      deleteFiles(this.cbdSelect.list.map((r) => r.id))
        .then((res) => {
          if (res.code == 200) {
            iMessage.success("操作成功!");
            this.findFiles();
    ...
    validateByL2(form = {}) {
      if (
        Array.isArray(form.levelTwoSumDTO.rawMaterial) &&
        form.levelTwoSumDTO.rawMaterial.length > 0
      ) {
        const keys = [];
        ...
        const rawMaterialFlag = form.levelTwoSumDTO.rawMaterial.every(
          (item) => {
            const flag = keys.every((key) => {
              if (!(!!item[key] || item[key] === 0 || item[key] === false)) {
                iMessage.warn(
                  this.language(
                    "QINGTIANXIEWANZHENGYUANCAILIAOSANJIAN",
                    "请填写完整原材料/散件"
                  )
                );
                return false;
      ...
      if (
        Array.isArray(form.levelTwoSumDTO.makeCost) &&
        form.levelTwoSumDTO.makeCost.length > 0
      ) {
        const keys = [];
        ...
        const makeCostFlag = form.levelTwoSumDTO.makeCost.every((item) => {
          const flag = keys.every((key) => {
            if (!(!!item[key] || item[key] === 0 || item[key] === false)) {
              iMessage.warn(
                this.language(
                  "QINGTIANXIEWANZHENGZHIZAOCHENGBEN",
                  "请填写完整制造成本"
                )
              );
              return false;
      ...
      if (
        Array.isArray(form.levelTwoSumDTO.discardCost) &&
        form.levelTwoSumDTO.discardCost.length > 0
      ) {
        const keys = [];
        ...
        const discardCostFlag = form.levelTwoSumDTO.discardCost.every(
          (item) => {
            const flag = keys.every((key) => {
              if (!(!!item[key] || item[key] === 0 || item[key] === false)) {
                iMessage.warn(
                  this.language(
                    "QINGTIANXIEWANZHENGBAOFEICHENGBEN",
                    "请填写完整报废成本"
                  )
                );
                return false;
      ...
      if (
        Array.isArray(form.levelTwoSumDTO.manageFee) &&
        form.levelTwoSumDTO.manageFee.length > 0
      ) {
        const keys = [];
      ...
      if (
        Array.isArray(form.levelTwoSumDTO.profit) &&
        form.levelTwoSumDTO.profit.length > 0
      ) {
        const keys = [];
        ...
        const profitFlag = form.levelTwoSumDTO.profit.every((item) => {
          const flag = keys.every((key) => {
            if (!(!!item[key] || item[key] === 0 || item[key] === false)) {
              iMessage.warn(
                this.language("QINGTIANXIEWANZHENLIRUN", "请填写完整利润")
              );
              return false;
            } else {
              return true;
    ...
    validateByL3(form = {}) {
      if (
        Array.isArray(form.levelThreeSumDTO.rawMaterial) &&
        form.levelThreeSumDTO.rawMaterial.length > 0
      ) {
        const keys = [];
        ...
        const rawMaterialFlag = form.levelThreeSumDTO.rawMaterial.every(
          (item) => {
            const flag = keys.every((key) => {
              if (!(!!item[key] || item[key] === 0 || item[key] === false)) {
                iMessage.warn(
      ...
      if (
        Array.isArray(form.levelThreeSumDTO.makeCost) &&
        form.levelThreeSumDTO.makeCost.length > 0
      ) {
        const keys = [];
      ...
      if (
        Array.isArray(form.levelThreeSumDTO.manageFee) &&
        form.levelThreeSumDTO.manageFee.length > 0
      ) {
        const keys = [];
      ...
      if (
        Array.isArray(form.levelThreeSumDTO.profit) &&
        form.levelThreeSumDTO.profit.length > 0
      ) {
        const keys = [];
    ...
    init(type) {
      ...
      if (
        this.partInfo.partProjectType === partProjTypes.DBYICHIXINGCAIGOU ||
        this.partInfo.partProjectType === partProjTypes.DBLINGJIAN ||
        this.partInfo.priceStatus == "DB"
      ) {
        ...
        if (type === "redraw") {
          ...
          this.allTableData.level = this.allTableData.level
            ? this.allTableData.level
            : this.partInfo.currentCbdLevel;
          this.allpagefrom["cbdLevel"] = this.allTableData.level
            ? this.allTableData.level
            : this.partInfo.currentCbdLevel;
        }
    ...
    getCostSummary(pageSize = 10, currPage = 1, type) {
      ...
      return new Promise((r) => {
        getCostSummary({ ...allpagefrom, rfqId: undefined })
          .then(async (res) => {
            if (res.data) {
              r(res.data);
              ...
              this.topTableData = this.translateDataTopData(
                cloneDeep(this.allTableData),
                data
              );
              this.$refs.components &&
                typeof this.$refs.components.partsQuotationss == "function" &&
                this.$refs.components.partsQuotationss(
                  this.partInfo.rfqId,
              ...
              this.summaryData.materialSummary = Array.isArray(
                this.allTableData.rawMaterial.records
              )
                ? this.allTableData.rawMaterial.records
                    .reduce(
                      (acc, cur) =>
                        math.add(acc, math.bignumber(cur.materialCost || 0)),
                      0
                    )
                    .toFixed(2)
                : "0.00";

              this.summaryData.productionSummary = Array.isArray(
                this.allTableData.makeCost.records
              )
                ? this.allTableData.makeCost.records
                    .reduce(
                      (acc, cur) =>
                        math.add(acc, math.bignumber(cur.totalCost || 0)),
                      0
                    )
                    .toFixed(2)
                : "0.00";
    ...
    getBzfreeAndYunshuFree() {
      return new Promise((r) => {
        packageTransport(this.allpagefrom.quotationId)
          .then((res) => {
            r(res.data);
          })
          .catch(() => {
    ...
    translateDataForRender(baseData) {
      try {
        ...
        switch (this.allpagefrom.cbdLevel) {
          ...
          case 2:
            data = {
              ...
              rawMaterial: baseData.levelTwoSumVO.rawMaterial
                ? baseData.levelTwoSumVO.rawMaterial
                : { records: [] },
              makeCost: baseData.levelTwoSumVO.makeCost
                ? baseData.levelTwoSumVO.makeCost
                : { records: [] },
              discardCost:
                Array.isArray(baseData.levelTwoSumVO.discardCost) &&
                baseData.levelTwoSumVO.discardCost.length > 0
              ...
              manageFee:
                Array.isArray(baseData.levelTwoSumVO.manageFee) &&
                baseData.levelTwoSumVO.manageFee.length > 0
              ...
              otherFee:
                Array.isArray(baseData.levelTwoSumVO.otherFee) &&
                baseData.levelTwoSumVO.otherFee.length > 0
              ...
              profit:
                Array.isArray(baseData.levelTwoSumVO.profit) &&
                baseData.levelTwoSumVO.profit.length > 0
            ...
            if (data["otherFee"]) {
              data["otherFee"].forEach((element) => {
                element["itemName"] = element["itemType"]
                  ? "Development Cost"
                  : "Tooling Cost";
              });
            }
          ...
          case 3:
            data = {
              ...
              rawMaterial: baseData.levelThreeSumVO.rawMaterial
                ? baseData.levelThreeSumVO.rawMaterial
                : { records: [] },
              makeCost: baseData.levelThreeSumVO.makeCost
                ? baseData.levelThreeSumVO.makeCost
                : { records: [] },
              discardCost:
                Array.isArray(baseData.levelThreeSumVO.discardCost) &&
                baseData.levelThreeSumVO.discardCost.length > 0
              ...
              manageFee:
                Array.isArray(baseData.levelThreeSumVO.manageFee) &&
                baseData.levelThreeSumVO.manageFee.length > 0
              ...
              otherFee:
                Array.isArray(baseData.levelThreeSumVO.otherFee) &&
                baseData.levelThreeSumVO.otherFee.length > 0
              ...
              profit:
                Array.isArray(baseData.levelThreeSumVO.profit) &&
                baseData.levelThreeSumVO.profit.length > 0
        ...
        data["level"] =
          this.allTableData && this.allTableData.level
            ? this.allTableData.level
            : this.partInfo.currentCbdLevel;
        // eslint-disable-next-line no-undef
        data["startProductDate"] = baseData.startProductDate
          ? moment(new Date(baseData.startProductDate)).format(
              "YYYY-MM-DD HH:mm:ss"
            )
          : "";
    ...
    translateDataTopData(a, b) {
      try {
        ...
        if (!this.isSteel && !this.roundIsOnlineBidding) {
          //钢材和onlinebidding是默认展示的totalPrice 不需要计算
          ...
          if (
            !this.Aprice.some(
              (key) =>
                data["tableData"][0][key] || data["tableData"][0][key] === 0
            )
          ) {
            this.$set(
              data["tableData"][0],
              "totalPrice",
              this.sourceResponseData.aprice
            );

            if (
              ["packageCost", "transportCost", "operateCost"].some(
                (key) => b[key] || b[key] === 0
              )
            ) {
              this.$set(
                data["tableData"][0],
                "totalPriceBprice",
                this.sourceResponseData.bprice
              );
            }
          }
        } else {
          data["persent"] =
            getPersent(data["tableData"][0]["totalPrice"], this.Aprice, a) ||
            [];
        }
    ...
    save(type) {
      ...
      if (
        ...
        if (this.isSkdLc) {
          if (
            +moment(this.$refs.skdCostSummary.skdStartProductDate) >
            +moment(this.allTableData.startProductDate)
          ) {
          ...
          if (
            !moment(this.allTableData.startProductDate).isAfter(
              moment(this.$refs.skdCostSummary.skdStartProductDate),
              "month"
            )
          ) {
    ...
    getCostSummaryDB() {
      ...
      return new Promise((r) => {
        getCostSummaryDB({ quotationId: this.partInfo.quotationId }).then(
          (res) => {
            if (res?.result) {
    ...
    updateCostSummaryDB() {
      const params = this.dbDetailList.map((item) => {
        return {
          ...
          seaPrice:
            item.sortOrder == 14 || item.sortOrder == 13 || item.sortOrder == 11
              ? null
              : item.seaPrice,
        };
      });
    ...
    handleDelByRawMaterial() {
      this.allTableData.rawMaterial.records =
        this.allTableData.rawMaterial.records.filter(
          (item) => !this.multipleSelectionByRawMaterial.includes(item)
        );
    ...
    handleDelByMakeCost() {
      this.allTableData.makeCost.records =
        this.allTableData.makeCost.records.filter(
          (item) => !this.multipleSelectionByMakeCost.includes(item)
        );
    ...
    forceComputeByL2() {
      if (this.allTableData.level == 2) {
        ...
        this.allTableData.rawMaterial.records.forEach((item) => {
          this.materialSummaryL2 = math.add(
            this.materialSummaryL2,
            math.bignumber(item.materialCost || 0)
          ); // 计算头表原材料/散件
          this.materialSummaryL2ByFalse = item.isSvwAssignPriceParts
            ? this.materialSummaryL2ByFalse
            : math.add(
                this.materialSummaryL2ByFalse,
                math.bignumber(item.materialCost || 0)
              );
        ...
        this.$set(
          this.topTableData.tableData[0],
          "materialSummary",
          this.materialSummaryL2.toFixed(2)
        );
        ...
        this.allTableData.makeCost.records.forEach((item) => {
          this.laborCostSummaryL2 = math.add(
            this.laborCostSummaryL2,
            math.bignumber(item.laborCost || 0)
          ); // 计算人工成本
          this.deviceCostSummaryL2 = math.add(
            this.deviceCostSummaryL2,
            math.bignumber(item.deviceCost || 0)
          ); // 计算设备成本
        });
        ...
        this.$set(
          this.topTableData.tableData[0],
          "productionSummary",
          math.add(this.laborCostSummaryL2, this.deviceCostSummaryL2).toFixed(2)
        );
      }
    },
    ...
    handleInputByRawMaterialL2(value, row) {
      if (row) {
        this.$set(
          ...
          math
            .evaluate(`${row.unitPrice || 0} * ${row.quantity || 0}`)
            .toFixed(2)
        );
        this.$set(
          row,
          ...
          math
            .evaluate(
              `${row.indirectMaterialCost || 0} * (${
                row.materialManageCostRate || 0
              } / 100)`
            )
            .toFixed(2)
        );
    ...
    handleInputByMakeCostL2(value, row) {
      if (row) {
        this.$set(
          ...
          math
            .evaluate(
              `(${row.deviceRate || 0} + ${row.directLaborRate || 0} * ${
                row.directLaborQuantity || 0
              }) * ${row.taktTime || 0} / 3600 / ${
                row.taktTimeNumber ? row.taktTimeNumber : 1
              } * (${row.indirectManufacturingRate || 0} / 100)`
            )
            .toFixed(2)
        );
        this.$set(
          ...
          math
            .evaluate(
              `${row.directLaborRate || 0} * ${
                row.directLaborQuantity || 0
              } * ${row.taktTime || 0} / 3600 / ${
                row.taktTimeNumber ? row.taktTimeNumber : 1
              } * (1 + (${row.indirectManufacturingRate || 0} / 100))`
            )
            .toFixed(2)
        );
        this.$set(
          ...
          math
            .evaluate(
              `${row.deviceRate || 0} * ${row.taktTime || 0} / 3600 / ${
                row.taktTimeNumber ? row.taktTimeNumber : 1
              } * (1 + (${row.indirectManufacturingRate || 0} / 100))`
            )
            .toFixed(2)
    ...
    handleInputByDiscardCostL2(value, row) {
      ...
      if (row) {
        this.$set(
          ...
          math
            .evaluate(
              `(${this.materialSummaryL2 || 0} + ${
                this.laborCostSummaryL2 || 0
              } + ${this.deviceCostSummaryL2 || 0}) / (1 - (${
                row.ratio || 0
              } / 100)) - (${this.materialSummaryL2 || 0} + ${
                this.laborCostSummaryL2 || 0
              } + ${this.deviceCostSummaryL2 || 0})`
            )
            .toFixed(2)
        );
      }
      ...
      this.allTableData.discardCost.forEach((item) => {
        this.scrapSummaryL2 = math.add(
          this.scrapSummaryL2,
          math.bignumber(item.amount || 0)
        ); // 计算报废成本
      });
    ...
    handleInputByManageFeeL2(value, row) {
      ...
      this.$set(
        this.allTableData.manageFee[0],
        "amount",
        math
          .evaluate(
            `${this.materialSummaryL2ByFalse || 0} * (${
              this.allTableData.manageFee[0].ratio || 0
            } / 100)`
          )
          .toFixed(2)
      );
      this.$set(
        this.allTableData.manageFee[1],
        "amount",
        math
          .evaluate(
            `(${
              math.add(this.laborCostSummaryL2, this.deviceCostSummaryL2) || 0
            }) * (${this.allTableData.manageFee[1].ratio || 0} / 100)`
          )
          .toFixed(2)
      );
      ...
      this.$set(
        ...
        math
          .add(
            math.bignumber(this.allTableData.manageFee[0].amount || 0),
            math.bignumber(this.allTableData.manageFee[1].amount || 0)
          )
          .toFixed(2)
      );
    },
    ...
    handleInputByProfitL2(value, row) {
      ...
      this.$set(
        ...
        "profitSummary",
        math
          .add(math.bignumber(this.allTableData.profit[0].amount || 0), 0)
          .toFixed(2)
      );
    },
    ...
    handleInputByRawMaterialL3(value, row) {
      if (row) {
        this.$set(
          ...
          math
            .evaluate(
              `(${row.unitPrice || 0} * ${row.roughWeight || 0}) - (${
                row.roughWeight || 0
              } - ${row.suttleWeight || 0}) * ${row.recycleUnitPrice || 0}`
            )
            .toFixed(2)
        );
        this.$set(
          ...
          math
            .evaluate(
              `${row.directMaterialCost || 0} / (1 - ${
                row.lossCostRate || 0
              } / 100) - ${row.directMaterialCost || 0}`
            )
            .toFixed(2)
        );
        this.$set(
          ...
          math
            .evaluate(
              `${row.indirectMaterialCostRatio || 0} / 100 * (${
                row.unitPrice || 0
              } * ${row.roughWeight || 0} + ${row.lossCost || 0})`
            )
            .toFixed(2)
        );
        this.$set(
          ...
          math
            .evaluate(
              `${row.directMaterialCost || 0} + ${row.lossCost || 0} + ${
                row.earlierLogisticsCost || 0
              } + ${row.indirectMaterialCost || 0}`
            )
            .toFixed(2)
        );
      ...
      this.allTableData.rawMaterial.records.forEach((item) => {
        materialSummary = math.add(
          materialSummary,
          math.bignumber(item.materialCost || 0)
        ); // 计算头表原材料/散件
        scrapSummary = math.add(
          scrapSummary,
          math.bignumber(item.lossCost || 0)
        ); // 计算报废成本
      });
      ...
      this.$set(
        this.topTableData.tableData[0],
        "materialSummary",
        math.subtract(materialSummary, materialScrapSummary).toFixed(2)
      );
      this.$set(
        this.allTableData.discardCost[0],
        "amount",
        scrapSummary.toFixed(2)
      );
      this.$set(
        this.topTableData.tableData[0],
        "scrapSummary",
      ...
      if (
        math.evaluate(
          `${this.summaryData.materialSummary || 0} + ${
            this.topTableData.tableData[0].productionSummary || 0
          }`
        ) === "0"
      ) {
        ...
        this.$set(
          ...
          math
            .evaluate(
              `${this.allTableData.discardCost[0].amount} / (${
                this.summaryData.materialSummary || 0
              } + ${this.topTableData.tableData[0].productionSummary || 0})`
            )
            .toFixed(2)
      ...
      this.$set(
        ...
        math
          .evaluate(
            `${this.summaryData.materialSummary} * (${this.allTableData.manageFee[0].ratio} / 100)`
          )
          .toFixed(2)
      );
      ...
      this.$set(
        this.allTableData.profit[0],
        "amount",
        math
          .evaluate(
            `${this.summaryData.materialSummary} * (${this.allTableData.profit[0].ratio} / 100)`
          )
          .toFixed(2)
      );
    ...
    handleInputByMakeCostL3(value, row) {
      if (row) {
        this.$set(
          ...
          math
            .evaluate(
              `(${row.perHourMachineCost || 0} + ${
                row.perHourLaborCost || 0
              } * ${row.workerCount || 0}) * ${row.perProduceTime} / 3600 / ${
                row.perCycleCount ? row.perCycleCount : 1
              }`
            )
            .toFixed(2)
        );
        this.$set(
          ...
          math
            .evaluate(
              `${row.directProduceCost || 0} / (1 - ${
                row.lossCostRate || 0
              } / 100) - ${row.directProduceCost || 0}`
            )
            .toFixed(2)
        );
        this.$set(
          ...
          math
            .evaluate(
              `(${row.directProduceCost || 0} + ${row.lossCost || 0} + ${
                row.produceSwitchCost || 0
              }) * (${row.indirectProduceCostRate || 0} / 100)`
            )
            .toFixed(2)
        );
        this.$set(
      ...
      this.allTableData.makeCost.records.forEach((item) => {
        productionSummary = math.add(
          productionSummary,
          math.bignumber(item.totalCost || 0)
        ); // 计算头表制造成本
        scrapSummary = math.add(
          scrapSummary,
          math.bignumber(item.lossCost || 0)
      ...
      this.$set(
        this.topTableData.tableData[0],
        "productionSummary",
        math.subtract(productionSummary, productionScrapSummary).toFixed(2)
      );
      this.$set(
        this.allTableData.discardCost[0],
        "amount",
      ...
      if (
        math.evaluate(
          `${this.summaryData.materialSummary || 0} + ${
            this.summaryData.productionSummary || 0
          }`
        ) === "0"
      ) {
        this.$set(this.allTableData.discardCost[0], "ratio", 0);
        ...
        this.$set(
          ...
          math
            .evaluate(
              `${this.allTableData.discardCost[0].amount || 0} / (${
      ...
      this.$set(
        ...
        math
          .evaluate(
            `${this.summaryData.productionSummary} * (${this.allTableData.manageFee[1].ratio} / 100)`
          )
          .toFixed(2)
      );
      ...
      this.$set(
        this.allTableData.profit[1],
        "amount",
        math
          .evaluate(
            `${this.summaryData.materialSummary} * (${this.allTableData.profit[1].ratio} / 100)`
          )
          .toFixed(2)
      );
    ...
    getCategoryDetail(code) {
      getCategoryDetail({
        ...
        if (res.code == 200) {
          switch (code) {
            case "MATERIAL": // 原材料
              this.materialDescribeOptions = Array.isArray(res.data)
                ? res.data.map((item) => ({
                    value: item.describe,
              ...
              if (Array.isArray(this.allTableData.rawMaterial.records)) {
                this.allTableData.rawMaterial.records.forEach((item) => {
                  if (item.partName === "MATERIAL")
                    this.$set(item, "autocompleteFn", (value, cb) =>
                      cb(this.materialDescribeOptions)
                    );
                });
              }
              break;
            case "COMPONENT": // 散件
              this.componentDescribeOptions = Array.isArray(res.data)
                ? res.data.map((item) => ({
                    value: item.describe,
                    unitCode: item.unitCode,
                  }))
              ...
              if (Array.isArray(this.allTableData.rawMaterial.records)) {
                this.allTableData.rawMaterial.records.forEach((item) => {
                  if (item.partName === "COMPONENT")
                    this.$set(item, "autocompleteFn", (value, cb) =>
                      cb(this.componentDescribeOptions)
                    );
                });
              }
              break;
            case "PRODUCTION_PROCESS": // 制造成本
              const productionProcessDescribeOptions = Array.isArray(res.data)
                ? res.data.map((item) => ({ value: item.describe }))
                : [];
              if (Array.isArray(this.titleCbzz))
                this.$set(this.titleCbzz[0], "autocompleteFn", (value, cb) =>
                  cb(productionProcessDescribeOptions)
                );
              if (Array.isArray(this.titleCbzzByL3))
                this.$set(
                  this.titleCbzzByL3[0],
                  "autocompleteFn",
                  (value, cb) => cb(productionProcessDescribeOptions)
                );
              break;
            default:
              break;
    ...
    handleSelectChangeByRawMaterial(value, row, key) {
      ...
      switch (key) {
        case "partName":
          if (value === "MATERIAL")
            this.$set(row, "autocompleteFn", (value, cb) =>
              cb(this.materialDescribeOptions)
            );
    ...
    updatePriceReducePlan() {
      updatePriceReducePlan({
        ...
        .then((res) => {
          if (res.code != 200) {
            iMessage.error(
              this.language("JIANGJIAJIHUAGENGXINSHIBAI", "降价计划更新失败")
            );
          }
        })
        .catch(() => {
