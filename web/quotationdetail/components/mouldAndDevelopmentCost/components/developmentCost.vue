      ...
      <template #header>
        <div class="header">
          <div>
            ...
            >
              <iFormItem
                class="item"
                :label="`${language(
                  developmentCostInfos[0].key,
                  developmentCostInfos[0].name
                )}`"
              >
          ...
          <div v-if="!disabled && !editDisabled" class="control">
            <iButton v-if="isAeko" @click="save" :loading="saveLoading">{{
              language("BAOCUN", "保存")
            }}</iButton>
      ...
      <div>
        ...
        >
          ...
          <template #unitPrice="scope">
            <iInput
              ...
              @input="handleInputByUnitPrice($event, scope.row)"
            />
        ...
        <iFormGroup class="subCost margin-top30" :row="4" inline>
          <template v-for="(info, $index) in subDevelopmentCostInfos">
            <iFormItem
              class="item"
              v-if="info.props == 'devFee'"
              :key="$index"
              :label="
                isAeko && info.languageKey
                  ? language(info.languageKey, info.languageName)
                  : language(info.key, info.name)
              "
            >
              <iText v-if="dataGroup['biddingDevFee']">{{
                dataGroup["biddingDevFee"] | thousandsFilter
              }}</iText>
              <iText v-else>{{
                dataGroup[info.props] | thousandsFilter
              }}</iText>
            </iFormItem>
            <iFormItem
              class="item"
            ...
            >
              <iInput
                v-if="
                  info.props === 'shareQuantity' && !disabled && !editDisabled
                "
...
import {
  ...
  developmentCostTableTitle as tableTitle,
  statesFilter,
} from "./data";
...
export default {
  ...
  data() {
    return {
      loading: false,
      developmentCostInfos,
      subDevelopmentCostInfos: cloneDeep(developmentCostInfos).slice(
        1,
        developmentCostInfos.length
      ),
      dataGroup: {},
      tableTitle,
      tableListData: [],
  ...
  methods: {
    getDevFee() {
      ...
      getDevFee({
        ...
        .then((res) => {
          if (res.code == 200) {
            this.tableListData = Array.isArray(res.data.devFeeInfoList)
              ? res.data.devFeeInfoList
              : [];
            this.$set(this.dataGroup, "devFee", res.data.devFee);
            this.$set(
              this.dataGroup,
              "rfqDevFeeTotal",
              res.data.rfqDevFeeTotal
            );
    ...
    updateTotal(e, scope) {
      ...
      this.tableListData.forEach((item) => {
        devFee = math.add(
          math.bignumber(devFee),
          math.bignumber(item.total || 0)
        );

        if (item.isShared == 1) {
          shareDevFee = math.add(
            math.bignumber(shareDevFee),
            math.bignumber(item.total || 0)
          );
        }
      });
      ...
      this.$set(
        ...
        +this.dataGroup.shareQuantity
          ? math
              .divide(shareDevFee, math.bignumber(this.dataGroup.shareQuantity))
              .toFixed(2)
          : "0"
      );
    },
