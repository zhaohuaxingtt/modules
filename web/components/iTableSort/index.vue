 ...
 * @LastEditTime: 2022-02-28 17:46:52
 * @LastEditors: YoHo
 * @Description: table组件
-->
<template>
  <div class="iFileTableList">
    ...
    >
      ...
      <el-table-column
        v-if="selection"
        type="selection"
        :width="selectConfig.width || 40"
        :align="selectConfig.align || 'center'"
        :header-align="selectConfig.headerAlign || 'center'"
        :selectable="selectConfig.selectable || selectable"
      ></el-table-column>
      <!----------------------支持自定义的index插槽------------------------>
      <el-table-column
        :fixed="indexFixed"
        v-if="index"
        type="index"
        :width="indexConfig.width || 50"
        :align="indexConfig.align || 'center'"
        :header-align="indexConfig.headerAlign || 'center'"
        :label="indexConfig.label || indexLabel"
      >
        ...
        >
          ...
          <template slot="header" slot-scope="scope">
            <div
              class="slotHeader"
              :class="{
                headerRequiredLeft: items._headerRequiredLeft,
                headerRequiredRight: items._headerRequiredRight,
              }"
            >
              <template>
                <span v-if="items.isHeaderSetting" class="isHeaderSetting">
                  <template v-for="(setting, index) in items.HeaderSettingList">
                    <span :key="index + '_setting'">{{
                      language(setting.key, setting.name)
                    }}</span>
                    <br
                      :key="index + '_setting_br'"
                      v-if="index + 1 < items.HeaderSettingList.length"
                    />
                  </template>
                </span>
                <span v-else class="noisHeaderSetting">{{
                  scope.column.label
                }}</span>
              </template>
              ...
              <i
                v-if="items.require"
                class="label-require margin-left3 margin-right3"
                >*</i
              >
              <el-popover
                placement="top"
                ...
                :visible-arrow="false"
                :disabled="!items.tipsKey"
              >
                <p
                  v-html="
                    items.tipsLang
                      ? language(items.tipsKey, items.tips)
                      : $t(items.tipsKey)
                  "
                ></p>
                <span slot="reference">
          ...
          <template slot-scope="row">
            <!-- <span class="flexRow"> -->
            <span
              :class="`openLinkText cursor ${activeItemsLink}`"
...
export default {
  ...
  methods: {
    ...
    handleSelectAll(selection) {
      ...
      !flag
        ? this.tableData.forEach((i) => {
            i.selectedBorder = !i.selectedBorder;
          })
        : "";
    ...
    handleSaveSetting(data) {
      ...
      if (
        this.$attrs.handleSaveSetting &&
        typeof this.$attrs.handleSaveSetting === "function"
      ) {
    ...
    handleResetSetting() {
      if (
        this.$attrs.handleResetSetting &&
        typeof this.$attrs.handleResetSetting === "function"
      ) {
        this.initTableSettingColumns();
        this.$attrs.handleResetSetting({
          data: cloneDeep(this.tableTitle),
          done: this.renewTableHeader,
        });
      }
    },
    getHeader() {
