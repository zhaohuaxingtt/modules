...
<template>
  ...
  >
    <el-table-column
      v-if="selection && hasList(tableTitle)"
      type="selection"
      width="55"
      align="center"
    ></el-table-column>
    <el-table-column
      v-if="indexKey && hasList(tableTitle)"
      type="index"
      width="50"
      align="center"
      label="#"
    ...
    <template v-for="items in tableTitle">
      ...
      <el-table-column
        ...
        v-if="items.props == 'totalPrice'"
        :prop="items.props"
        :label="
          items.key
            ? lang
              ? language(items.key, items.name)
              : $t(items.key)
            : items.name
        "
      >
        <template slot-scope="row">
          ...
          <span v-else>{{
            isEmptyPriceCompute(Aprice, row.row)
              ? row.row.totalPrice
              : getAallPrice(Aprice, row.row)
          }}</span>
        </template>
      </el-table-column>
      <!----------------------B价------------------------>
      <el-table-column
        ...
        :width="items.width"
        :show-overflow-tooltip="items.tooltip"
        v-else-if="items.props == 'totalPriceBprice'"
        :prop="items.props"
        :label="
          items.key
            ? lang
      ...
      >
        <template slot-scope="row">
          ...
          <span v-else-if="row.row.biddingBPrice">{{
            row.row.biddingBPrice
          }}</span>
          <span v-else>{{
            isEmptyPriceCompute(Aprice, row.row) &&
            isEmptyPriceCompute(
              ["packageCost", "transportCost", "operateCost"],
              row.row
            )
              ? row.row.totalPriceBprice
              : getBallPrice(Bprice, row.row)
          }}</span>
        </template>
      </el-table-column>
      <!----------------------需要高亮的列并且带有打开详情事件------------------------>
      <el-table-column
        ...
        :width="items.width"
        :show-overflow-tooltip="items.tooltip"
        v-else-if="items.props == activeItems"
        :prop="items.props"
        :label="`${items.seq ? items.seq + ' ' : ''}${
          items.key
            ? lang
              ? language(items.key, items.name)
              : $t(items.key)
      ...
      <el-table-column
        ...
        :width="items.width"
        :show-overflow-tooltip="items.tooltip"
        v-else-if="items.props == 'isSvwAssignPriceParts'"
        :prop="items.props"
        :label="`${items.seq ? items.seq + ' ' : ''}${
          items.key
            ? lang
      ...
      >
        <template slot-scope="row">
          ...
          <iSelect
            v-else
            v-model="row.row[items.props]"
            @change="$emit('handleChangeByIsSvwAssignPriceParts')"
          >
            <el-option
              :value="options.value"
              v-for="(options, optionIndex) in items.options"
              :key="optionIndex"
              :label="options.label"
            ></el-option>
          </iSelect>
      ...
      <el-table-column
        ...
        :show-overflow-tooltip="items.tooltip"
        v-else-if="
          items.props == 'specialDeviceCost' && items.type === 'select'
        "
        :prop="items.props"
        :label="`${items.seq ? items.seq + ' ' : ''}${
          items.key
      ...
      <el-table-column
        ...
        align="center"
        :width="items.width"
        v-else-if="items.props == 'paixu'"
      >
        <tempalte slot-scope="scope">
          <span
            @click="updateSlot(scope.row, 0)"
            v-if="scope.row.recordId && parseInt(scope.row.recordId)"
          >
      ...
      <el-table-column
        ...
        v-else
        :label="`${items.seq ? items.seq + ' ' : ''}${
          items.key ? groupTileTranslate(items) : items.name
        }`"
        :prop="items.props"
        :label-class-name="items.labelClassName"
      >
        <template slot="header">
          <el-tooltip
            effect="light"
            :content="`${items.seq ? items.seq + ' ' : ''}${
              items.key ? groupTileTranslate(items) : items.name
            }`"
          >
            <span>
        ...
        <template v-if="items.list && items.list.length > 0">
          <template v-for="(itemss, indexs) in items.list">
            <el-table-column
              v-if="selection && itemss.props == 'selection'"
              :key="indexs"
              type="selection"
              width="55"
              align="center"
            ></el-table-column>
            <el-table-column
              v-else-if="indexKey && itemss.props == 'index'"
              :key="indexs"
              type="index"
              width="50"
              align="center"
            ...
            <!----------------------------物料费用展示------------------------>
            <el-table-column
              v-else-if="itemss.props == 'materialPersent'"
              :key="indexs"
              align="center"
              :width="itemss.width"
              :show-overflow-tooltip="itemss.tooltip"
              :label="
                itemss.key
                  ? lang
                    ? language(itemss.key, items.name)
                    : $t(itemss.key)
                  : itemss.name
              "
              :prop="itemss.props"
            >
              <template slot-scope="scope">
                <span v-if="notEdit">
                  {{
                    getPersion(
                      scope.row.indirectMaterialCost,
            ...
            <el-table-column
              ...
              :label="
                itemss.key
                  ? lang
                    ? language(itemss.key, itemss.name)
                    : $t(itemss.key)
                  : itemss.name
              "
              :prop="itemss.props"
            >
              ...
              <tempalte slot-scope="scope">
                ...
                <template v-else-if="itemss.type == 'input'">
                  <iInput
                    v-model="scope.row[itemss.props]"
                    v-if="!notEdit"
                    @input="
                      handleInput(
                        $event,
        ...
        <template v-if="!items.list" slot-scope="scope">
          <template v-if="items.type == 'select'">
            <iSelect
              ...
              v-if="!notEdit"
              @change="
                $emit('handleSelectChange', $event, scope.row, items.props)
              "
            >
              <el-option
                :value="options.value"
                v-for="(options, optionIndex) in items.options"
                :key="optionIndex"
                :label="options.name || options.label"
              ></el-option>
            </iSelect>
            <span v-else>{{
              items.showLabel
                ? showLabel(scope.row[items.props], items.options)
          ...
          <template v-else-if="items.type == 'autocomplete'">
            <el-autocomplete
              ...
              v-model="scope.row[items.props]"
              :fetch-suggestions="
                scope.row.autocompleteFn || items.autocompleteFn
              "
              @select="
                $emit(
          ...
          <template v-else-if="items.type == 'inputRate'">
            <div class="flexVerticalCenter" v-if="!notEdit">
              <iInput
                v-model="scope.row[items.props]"
                @input="handleInputByRate($event, scope.row, items.props)"
              ></iInput
              ><span class="margin-left5">%</span>
            </div>
            <span v-else>{{ scope.row[items.props] }}</span>
          </template>
          <!---------------------------bidding---------------------------->
          <template v-else-if="items.props.indexOf('items') > -1">
            <span v-if="scope.$index == 0">{{
              scope.row[items.props].quotation
            }}</span>
            <span v-if="scope.$index == 1">{{
              fixTo2(scope.row[items.props].bidding)
            }}</span>
            <span
              v-if="scope.$index == 2"
              :class="{ redClass: scope.row[items.props].isColor }"
              >{{ scope.row[items.props].result }}</span
            >
          </template>
          <template v-else>
            <span v-if="items.type === 'inputRate' && !notEdit"
              >{{ scope.row[items.props]
              }}{{
                (scope.row[items.props] == null ||
                  scope.row[items.props] == "") &&
                scope.row[items.props] !== 0
                  ? ""
                  : "%"
              }}</span
            >
            <span v-else>{{
              typeof filterProps[items.props] === "function"
                ? filterProps[items.props](scope.row[items.props])
                : scope.row[items.props]
            }}</span>
          </template>
        </template>
...
import {
  ...
  Aprice,
  Bprice,
} from "../../../quotationdetail/components/costsummary/components/data";
...
export default {
  ...
  methods: {
    ...
    getBallPrice(a, b) {
      console.log(
        "(Aprice.every(key => b[key] === null || b[key] === undefined))",
        Aprice.every((key) => b[key] === null || b[key] === undefined)
      );

      const result = Aprice.every((key) => !b[key] || b[key] == 0)
        ? getAallPrice(a, b, "B", b.totalPrice)
        : getAallPrice(a, b);
    ...
    handleInput(value, row, key, column, precision = 2) {
